...
void compute(int arg) {
    if (arg < 2) {
        //pthread_mutex_lock(&gMtx); removi esse trecho que causava travamento pois era o mesmo valor lock que causava
        //a thread olhar para a variavel e estar em lock da função que chamou essa, troquei o argumento para uma nova area
        // e tive meu problema resolvido.
...
void* compute_thread(void* arg) {
    ...
    // removi o lock daqui para deixar o programa mais rapido o mutex esta apenas na variavel global
    gValue = 0;
    compute(*((int*)arg));
    *ret = gValue;
    return ret;
