...
sem_t sem_lista;
// O worker 2 estava ocupandp um mutex, que por sua vez o worker 1 aguardava o mutex ser liberado
// E o mutex do worker 2 so seria liberado se o semaforo ocupado pelo worker 1 que aguarda o mutex ocupado
// pelo worker 2 gerando um impasse Deadlock, uma espera circular um esperando o outro.
void *worker1_func(void *arg) {
    for (int i = 0; i < WORKER_LOOPS; ++i) {
        sem_wait(&sem_lista); // SEMAFORO DE ESPERA
        ...
        sem_post(&sem_lista); // SEMAFORO DE SAIDA
